...
{
    public class ApproveDiscountApprovalCommandHandlerTests
    {
        ...
        [Fact]
        public async Task Handle_ApprovesPendingApproval_AndUnlocksQuotation()
        {
            using var context = CreateContext();
            var (approval, approverUserId) = await SeedPendingApproval(context);
            var mapper = NewMapper();

            var handler = new ApproveDiscountApprovalCommandHandler(
                context,
                mapper,
                NullLogger<ApproveDiscountApprovalCommandHandler>.Instance);
            ...
            Assert.NotNull(saved.ApprovalDate);
        ...
        {
            using var context = CreateContext();
            var (approval, approverUserId) = await SeedApprovedApproval(context);
            var mapper = NewMapper();
        ...
        [Fact]
        public async Task Handle_ThrowsException_WhenUnauthorizedRole()
        {
            using var context = CreateContext();
            var (approval, _) = await SeedPendingApproval(context);
            var unauthorizedUserId = Guid.NewGuid();
            SeedUser(context, unauthorizedUserId, "SalesRep");
            var mapper = NewMapper();
            ...
            await Assert.ThrowsAsync<UnauthorizedApprovalActionException>(() => handler.Handle(command));
        }
        ...
        private static async Task<(DiscountApproval, Guid)> SeedApprovedApproval(AppDbContext context)
        {
            var (approval, approverUserId) = await SeedPendingApproval(context);
            approval.Status = ApprovalStatus.Approved;
            approval.ApproverUserId = approverUserId;
            approval.ApprovalDate = DateTimeOffset.UtcNow;
