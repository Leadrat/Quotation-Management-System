...
{
    public class GetRefundsByQuotationQueryHandler
    {
        private readonly IAppDbContext _db;

        public GetRefundsByQuotationQueryHandler(IAppDbContext db)
        {
            _db = db;
        }

        public async Task<List<RefundDto>> Handle(GetRefundsByQuotationQuery query)
        {
            var refunds = await _db.Refunds
                .Include(r => r.RequestedByUser)
                .Include(r => r.ApprovedByUser)
                .Where(r => r.QuotationId == query.QuotationId)
