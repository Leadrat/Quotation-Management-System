              ...
              <li key={notification.notificationId}>
                <DropdownItem
                  ...
                  className={`flex gap-3 rounded-lg border-b border-gray-100 p-3 px-4.5 py-3 hover:bg-gray-100 dark:border-gray-800 dark:hover:bg-white/5 ${
                    !notification.isRead ? "bg-gray-50 dark:bg-white/5" : ""
                  }`}
                  href={notification.entityLinkUrl || "#"}
                >
                  <span className="relative block w-full h-10 rounded-full z-1 max-w-10">
                    <div className="w-10 h-10 rounded-full bg-brand-500 flex items-center justify-center text-white text-sm font-medium">
                      {notification.eventType?.charAt(0)?.toUpperCase() || "N"}
                    </div>
                    {!notification.isRead && (
